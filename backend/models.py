...
class AdherenceLog(Base):
    """
    Records every medication-task completion event for compliance tracking.
    action: 'on_time' | 'late' | 'missed'
    """
    ...
    action          = Column(String, nullable=False)       # on_time | late | missed
    scheduled_time  = Column(String, nullable=True)        # "08:00 AM"
    completed_time  = Column(String, nullable=True)        # ISO timestamp
    task_date       = Column(String, nullable=True)        # YYYY-MM-DD
    timestamp       = Column(String, nullable=False)


class AgentAlert(Base):
    """
    Proactive alerts generated by the medication agent.
    alert_type: 'low_stock' | 'missed_dose' | 'adherence_drop' | 'refill_reminder'
    """
