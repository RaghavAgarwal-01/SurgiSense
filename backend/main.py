...
# Runs every deploy on Render — safe because all statements use IF NOT EXISTS.
# Ensures discharge_summaries, medicines, medication_logs, adherence_logs,
# agent_alerts and all new columns exist before any request is served.
def _run_startup_migrations():
    import psycopg2
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        logger.warning("DATABASE_URL not set — skipping startup migration")
        return
    CREATE_TABLES = [
        ...
        """CREATE TABLE IF NOT EXISTS discharge_summaries (
            ...
            surgery_type VARCHAR, surgery_date VARCHAR,
            surgery_phase VARCHAR DEFAULT 'post',
            icd10_code VARCHAR, cpt_code VARCHAR,
            bp_sys INTEGER, bp_dia INTEGER, heart_rate INTEGER,
            spo2 INTEGER, temperature VARCHAR, hemoglobin VARCHAR,
        ...
        """CREATE TABLE IF NOT EXISTS medicines (
            ...
            total_quantity INTEGER, current_quantity INTEGER,
        ...
        """CREATE TABLE IF NOT EXISTS adherence_logs (
            id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id),
            task_id INTEGER, medicine_id INTEGER,
            action VARCHAR NOT NULL, scheduled_time VARCHAR,
            completed_time VARCHAR, task_date VARCHAR,
            timestamp VARCHAR NOT NULL)""",
        """CREATE TABLE IF NOT EXISTS agent_alerts (
            id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id),
            alert_type VARCHAR NOT NULL, message VARCHAR NOT NULL,
            data_json TEXT, is_read INTEGER DEFAULT 0,
            created_at VARCHAR NOT NULL)""",
    ]
    ALTER_COLUMNS = [
        ("patient_profiles", "age",                   "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS age INTEGER"),
        ("patient_profiles", "gender",                "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS gender VARCHAR"),
        ("patient_profiles", "surgery_phase",         "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS surgery_phase VARCHAR DEFAULT 'post'"),
        ("patient_profiles", "icd10_code",            "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS icd10_code VARCHAR"),
        ("patient_profiles", "cpt_code",              "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS cpt_code VARCHAR"),
        ("patient_profiles", "payer_id",              "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS payer_id VARCHAR"),
        ("patient_profiles", "recovery_days_total",   "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS recovery_days_total INTEGER DEFAULT 90"),
        ("patient_profiles", "pdf_upload_date",       "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS pdf_upload_date VARCHAR"),
        ("patient_profiles", "next_appointment_date", "ALTER TABLE patient_profiles ADD COLUMN IF NOT EXISTS next_appointment_date VARCHAR"),
        ("recovery_tasks",   "task_date",             "ALTER TABLE recovery_tasks ADD COLUMN IF NOT EXISTS task_date VARCHAR"),
        ("recovery_tasks",   "is_critical",           "ALTER TABLE recovery_tasks ADD COLUMN IF NOT EXISTS is_critical INTEGER DEFAULT 0"),
        ("medicines",        "dose_amount",           "ALTER TABLE medicines ADD COLUMN IF NOT EXISTS dose_amount INTEGER DEFAULT 1"),
        ("medicines",        "total_quantity",        "ALTER TABLE medicines ADD COLUMN IF NOT EXISTS total_quantity INTEGER"),
        ("medicines",        "current_quantity",      "ALTER TABLE medicines ADD COLUMN IF NOT EXISTS current_quantity INTEGER"),
        ("adherence_logs",   "scheduled_time",        "ALTER TABLE adherence_logs ADD COLUMN IF NOT EXISTS scheduled_time VARCHAR"),
        ("adherence_logs",   "completed_time",        "ALTER TABLE adherence_logs ADD COLUMN IF NOT EXISTS completed_time VARCHAR"),
        ("adherence_logs",   "task_date",             "ALTER TABLE adherence_logs ADD COLUMN IF NOT EXISTS task_date VARCHAR"),
        ("adherence_logs",   "task_id",               "ALTER TABLE adherence_logs ADD COLUMN IF NOT EXISTS task_id INTEGER"),
        ("adherence_logs",   "medicine_id",           "ALTER TABLE adherence_logs ADD COLUMN IF NOT EXISTS medicine_id INTEGER"),
        ("agent_alerts",     "data_json",             "ALTER TABLE agent_alerts ADD COLUMN IF NOT EXISTS data_json TEXT"),
    ]
    try:
        ...
        for sql in CREATE_TABLES:
            try:
                cur.execute(sql)
                logger.info(f"startup migration: table ensured")
            except Exception as e:
                logger.error(f"startup migration create error: {e}")
        for _, col, sql in ALTER_COLUMNS:
            try:
                cur.execute(sql)
            except Exception:
