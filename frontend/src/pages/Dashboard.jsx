...
export default function Dashboard() {
  ...
  // 1. Replaced static tasks with state
  ...
  // 2. Added toggle function to make tasks clickable
  const toggleTaskStatus = (taskId) => {
    if (taskId === 'placeholder') return; // Don't toggle the placeholder
  ...
  const handleDischargeUpload = async (file) => {
    ...
    try {
      ...
      // Step B: Generate the dynamic schedule using the extracted data
      try {
        ...
        // Fallback or error handling for task generation could go here
      }
  ...
  return (
    <div className="min-h-screen bg-[#D3D0BC]">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* 3. Recovery Timeline Cards - Now Map over State and are Clickable */}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div>
                      <h3 className={`font-semibold ${task.status === 'completed' ? 'text-[#9AA7B1] line-through' : 'text-[#3E435D]'}`}>
                        {task.title}
                      </h3>
        ...
        <section className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          {!digitizedData && (
            <div className="border-2 border-dashed border-[#CBC3A5] rounded-xl p-6 text-center">
              ...
              >
                {loadingRecord ? "Processing AI..." : "Upload & Digitize"}
              </label>
            </div>
          )}
          ...
          {digitizedData && (
            <>
              ...
              <button className="mt-4 text-[#3E435D] font-medium flex items-center gap-1 hover:gap-2 transition-all cursor-default">
